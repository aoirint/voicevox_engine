...
# Ubuntu 22.04
.PHONY: build-linux-docker-ubuntu22.04
build-linux-docker-ubuntu22.04:
	docker buildx build . \
		-t voicevox/voicevox_engine:cpu-ubuntu22.04-latest \
		--target runtime-env \
		--progress plain \
		--build-arg BASE_IMAGE=ubuntu:22.04 \
		--build-arg BASE_RUNTIME_IMAGE=ubuntu:22.04 \
...
.PHONY: run-linux-docker-ubuntu22.04
run-linux-docker-ubuntu22.04:
	docker run --rm -it \
		-p '127.0.0.1:50021:50021' $(ARGS) \
		voicevox/voicevox_engine:cpu-ubuntu22.04-latest $(CMD)

.PHONY: build-linux-docker-nvidia-ubuntu22.04
build-linux-docker-nvidia-ubuntu22.04:
	docker buildx build . \
		-t voicevox/voicevox_engine:nvidia-ubuntu22.04-latest \
		--target runtime-nvidia-env \
		--progress plain \
		--build-arg BASE_IMAGE=ubuntu:22.04 \
		--build-arg BASE_RUNTIME_IMAGE=nvidia/cuda:11.6.2-cudnn8-runtime-ubuntu22.04 \
...
.PHONY: run-linux-docker-nvidia-ubuntu22.04
run-linux-docker-nvidia-ubuntu22.04:
	docker run --rm -it \
		--gpus all \
		-p '127.0.0.1:50021:50021' $(ARGS) \
		voicevox/voicevox_engine:nvidia-ubuntu22.04-latest $(CMD)
